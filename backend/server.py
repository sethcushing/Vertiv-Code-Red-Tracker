...
# Import routes
from routes import auth, initiatives, business_outcomes, dashboard, admin, config
# Note: seed module removed from production to prevent data overwrites
...
@api_router.get("/")
async def root():
    return {"message": "Code Red Initiatives API", "version": "4.2.2", "seed_disabled": True}

# Health check endpoint (for Kubernetes/Koyeb liveness/readiness probes)
@app.get("/health")
async def health_check():
    return {"status": "healthy", "version": "4.2.2"}

# Version endpoint for deployment verification
@api_router.get("/version")
async def get_version():
    return {"version": "4.2.2", "db_name": db_name}
